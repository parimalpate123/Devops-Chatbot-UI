(function () {
    ...
    $(function () {
        ...
        socket.on(replyChannel, function (jsonObj) {
            ...
            if(count==0)
            {
                appendMessage("Hi I am DevOps Genie. I am currently in development, please feel free to ask me anything","bot","default");
                count++;
            }
            else{
            jsonObj.message.forEach(rep => {
                     console.log("test");
                     console.log(rep.recipient_id+"====>"+rep.text);
                     appendMessage(rep.text, "bot",rep.recipient_id );
        ...
        $('.messages').on('click', "span.feedback_yes", function () {
            ...
            var feedbackYesLog="YES! ID: " + replyId + " | message: " + message + " | intent: " + intent;
            var currDate=new Date();
            feedbackYesLog+=" : "+currDate;
            //Write To Log File
            /*var fso = new ActiveXObject("Scripting.FileSystemObject");
            var FileObject = fso.OpenTextFile("C:\\LogFile.txt", 8, true,0);
            FileObject.write(feedbackLog);
            FileObject.close();*/

            
            console.log(feedbackYesLog);
        ...
        $('.messages').on('click', "span.feedback_no", function () {
            ...
            var message = $(this).parent().parent().parent().parent().parent().parent().find(".message[messageId='" + replyId + "']").find(".text").text();
            //Feedback No
            var feedbackNoLog="NO! ID: " + replyId + " | message: " + message + " | intent: " + intent;
            var currDate=new Date();
            feedbackNoLog+=" : "+currDate;


            console.log(feedbackNoLog);
            var wrapper = $(this).parent().parent().parent().parent();
            var wrapperParent = $(this).parent().parent().parent().parent().parent();
